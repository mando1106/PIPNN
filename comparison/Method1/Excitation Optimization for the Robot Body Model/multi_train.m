...
max_trials = 30;  % 训练次数
% 结果结构体数组
results(max_trials) = struct('coeff', [], 'cond_num', [], 'time', []);
total_tic = tic;  % 外层开始计时
for trial = 1:max_trials
    ...
    trial_time = toc(trial_tic);
    
    % 保存本次结果
    results(trial).coeff = Coefficient_ExTra;
    results(trial).cond_num = Y_total_kbese_i(end);
    results(trial).time = trial_time;
...
% 保存结果到文件夹
data_dir = fullfile(pwd, 'data_tlbo_exp');
...
% 保存每次trial的cond和时间
fid = fopen(fullfile(data_dir, 'tlbo_results.txt'), 'wt');
fprintf(fid, 'Trial\tCondition\tTime(s)\n');
for i = 1:max_trials
    fprintf(fid, '%d\t%.6f\t%.3f\n', i, results(i).cond_num, results(i).time);
end
fclose(fid);

% 保存所有系数到 mat 文件
save(fullfile(data_dir, 'tlbo_all_results.mat'), 'results');
